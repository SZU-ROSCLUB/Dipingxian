...
inline float GetPixelValue(const cv::Mat& img, float x, float y) {
    ...
    const T* data = &img.at<T>(floor(y), floor(x));  // 获取基准像素指针   目标点左上角那个像素
    float xx = x - floor(x);  // 计算小数部分权重
    float yy = y - floor(y);
    return float(
        (1 - xx) * (1 - yy) * data[0] +  // 权重 * 左上角像素(Q11)
        xx * (1 - yy) * data[1] +        // 权重 * 右上角像素(Q21)
        (1 - xx) * yy * data[img.step / sizeof(T)] + // 权重 * 左下角像素(Q12)
        xx * yy * data[img.step / sizeof(T) + 1]    // 权重 * 右下角像素(Q22)
    );
}
